                        ...
                        <tbody>
                            {posts.map(post => (
                                <tr key={post.id} className={`border-b border-gray-700 hover:bg-gray-700/30 transition-colors ${post.blocked ? 'bg-yellow-900/40 opacity-80' : ''}`}>
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-2">
                                            <span>{visiblePasswordId === post.id ? post.password : '••••••••'}</span>
                                            <button onClick={() => setVisiblePasswordId(visiblePasswordId === post.id ? null : post.id)} className="text-gray-400 hover:text-white">
                                                <EyeIcon className="w-4 h-4" />
                                            </button>
                                        </div>
                                    </td>
...
export const ReportsContent: React.FC<{ events: MonitoringEvent[]; posts: ServicePost[]; currentUser: Company }> = ({ events, posts, currentUser }) => {
    ...
    const generatePDF = async () => {
        ...
        // 4. Table
        const tableColumn = ["ID", "Posto", "Evento", "Data/Hora", "Comentário", "Status"];
        const tableRows: any[] = [];
    ...
    return (
        <>
            ...
            <div className="bg-gray-800/50 backdrop-blur-sm border border-gray-700 rounded-lg overflow-hidden">
                <h3 className="p-4 text-lg font-semibold text-white border-b border-gray-700">Relatório de Eventos ({filteredEvents.length} eventos)</h3>
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left text-gray-300">
                        <thead className="text-xs text-gray-400 uppercase bg-gray-700/50">
                            <tr>
